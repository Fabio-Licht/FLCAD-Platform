...
class _BlockingRepository implements BackendProvisioningRepository {
    ...
    switch (arguments.first) {
      ...
      case 'mapper':
        ...
        for (final name in ['cameras.bin', 'images.bin', 'points3D.bin']) {
          await write(
            '$output${Platform.pathSeparator}2${Platform.pathSeparator}$name',
          );
        }
        break;
      case 'image_undistorter':
        final output = valueAfter('--output_path');
        await write(
          '$output${Platform.pathSeparator}images${Platform.pathSeparator}a.jpg',
        );
        for (final name in ['cameras.bin', 'images.bin', 'points3D.bin']) {
          await write(
            '$output${Platform.pathSeparator}sparse${Platform.pathSeparator}$name',
          );
        }
        break;
...
class _Backend implements ReconstructionBackend {
  _Backend(this.label, {this.backendId = 'colmap'});
...
void main() {
  test(
    'shared coordinator queue keeps persistence memory and backend aligned',
    () async {
      final root = await Directory.systemTemp.createTemp('flcad-coordinate-');
      addTearDown(() => root.delete(recursive: true));
      Future<File> executable(String folder) async => File(
        '${root.path}${Platform.pathSeparator}$folder${Platform.pathSeparator}'
        '$_executableName',
      ).create(recursive: true);
      final firstExecutable = await executable('first');
      final secondExecutable = await executable('second');
      ...
      final firstBackend = _Backend('first');
      final secondBackend = _Backend('second');
      final first = ColmapExternalActivationCoordinator(
        provisioningManager: provisioning,
        backendManager: backends,
        prepareBackend: (record, {required allowUncertifiedExternal}) async =>
            firstBackend,
      );
      final second = ColmapExternalActivationCoordinator(
        provisioningManager: provisioning,
        backendManager: backends,
        prepareBackend: (record, {required allowUncertifiedExternal}) async =>
            secondBackend,
      ...
      final firstOperation = first.activateExternal(
        executablePath: firstExecutable.path,
        authorized: true,
        allowExperimental: true,
      );
      await repository.firstSaveStarted.future;
      final secondOperation = second.activateExternal(
      ...
      final persisted = repository.records.single;
      expect(
        persisted.executablePath,
        await secondExecutable.resolveSymbolicLinks(),
      );
      expect(
        provisioning.installations.single.executablePath,
        persisted.executablePath,
      );
      expect(backends.get('colmap'), same(secondBackend));
      expect(repository.saveCalls, 2);
    },
  );

  test('inverted preparation completion remains consistent', () async {
    final root = await Directory.systemTemp.createTemp('flcad-inverted-');
    addTearDown(() => root.delete(recursive: true));
    Future<File> executable(String folder) => File(
      '${root.path}${Platform.pathSeparator}$folder${Platform.pathSeparator}'
      '$_executableName',
    ).create(recursive: true);
    final firstExecutable = await executable('first');
    ...
    final backends = ReconstructionBackendManager();
    final firstReady = Completer<ReconstructionBackend>();
    final secondReady = Completer<ReconstructionBackend>();
    final firstBackend = _Backend('first');
    final secondBackend = _Backend('second');
    final first = ColmapExternalActivationCoordinator(
      provisioningManager: provisioning,
      backendManager: backends,
      prepareBackend: (record, {required allowUncertifiedExternal}) =>
          firstReady.future,
    );
    final second = ColmapExternalActivationCoordinator(
      provisioningManager: provisioning,
    ...
    secondReady.complete(secondBackend);
    await secondOperation;
    firstReady.complete(firstBackend);
    await firstOperation;

    final persisted = repository.memory.single;
    expect(
      persisted.executablePath,
      await firstExecutable.resolveSymbolicLinks(),
    );
    expect(
      provisioning.installations.single.executablePath,
      persisted.executablePath,
    );
    expect(backends.get('colmap'), same(firstBackend));
  });

  test('prepared candidate is single-use and manager-bound', () async {
    final root = await Directory.systemTemp.createTemp('flcad-candidate-');
    addTearDown(() => root.delete(recursive: true));
    final executable = await File(
      '${root.path}${Platform.pathSeparator}$_executableName',
    ).create();
    BackendProvisioningManager manager(_Repository repository) => _manager(
      repository: repository,
      downloader: _NeverDownloader(),
      installer: _NeverInstaller(),
      selfTest: _SelfTest(),
    ...
    final firstManager = manager(firstRepository);
    final secondManager = manager(_Repository());
    final backends = ReconstructionBackendManager();
    ColmapExternalActivationCoordinator coordinator(
      BackendProvisioningManager provisioning,
    ...
    final candidate = await firstManager.prepareExisting(
      'colmap',
  ...
  test('candidate stays consumed when persistence fails', () async {
    final root = await Directory.systemTemp.createTemp('flcad-consumed-');
    addTearDown(() => root.delete(recursive: true));
    final executable = await File(
      '${root.path}${Platform.pathSeparator}$_executableName',
    ).create();
    final repository = _Repository()..saveFailure = StateError('save failed');
    final provisioning = _manager(
      repository: repository,
      downloader: _NeverDownloader(),
      installer: _NeverInstaller(),
  ...
  test('coordinator rejects non-COLMAP backend before persistence', () async {
    final root = await Directory.systemTemp.createTemp('flcad-wrong-id-');
    addTearDown(() => root.delete(recursive: true));
    final executable = await File(
      '${root.path}${Platform.pathSeparator}$_executableName',
    ).create();
    ...
    final coordinator = ColmapExternalActivationCoordinator(
      ...
      backendManager: backends,
      prepareBackend: (record, {required allowUncertifiedExternal}) async =>
          _Backend('wrong', backendId: 'foundation'),
    );

    await expectLater(
      coordinator.activateExternal(
        executablePath: executable.path,
        authorized: true,
        allowExperimental: true,
      ),
    ...
    expect(repository.memory, isEmpty);
    expect(provisioning.installations, isEmpty);
    expect(backends.contains('colmap'), isFalse);
  });
  ...
  test(
    'external registration is authorized, persistent and never certified',
    () async {
      ...
      final storage = File(
        '${temp.path}${Platform.pathSeparator}installations.json',
      );
  ...
  test(
    'registration rejects relative, missing and wrongly named executables',
    () async {
      final temp = await Directory.systemTemp.createTemp(
        'flcad-invalid-external-',
      );
      addTearDown(() => temp.delete(recursive: true));
  ...
  test(
    'activation requires opt-in and uses the exact executable end to end',
    () async {
      final temp = await Directory.systemTemp.createTemp(
        'flcad-activate-external-',
      );
      addTearDown(() => temp.delete(recursive: true));
      final executable = await File(
      ...
      await File(
        '${images.path}${Platform.pathSeparator}a.jpg',
      ).writeAsBytes([1]);
  ...
  test(
    'activates only a certified managed executable confined to its root',
    () async {
  ...
  test(
    'low-level COLMAP backend rejects implicit relative executables',
    () async {
      final runner = _PipelineRunner();
      await expectLater(
        ColmapBackend(
  ...
  test(
    'external discovery rejects a canonical alias with a wrong name',
    () async {
      final temp = await Directory.systemTemp.createTemp(
        'flcad-external-alias-',
      );
      addTearDown(() => temp.delete(recursive: true));
      final executable = await File(
        '${temp.path}${Platform.pathSeparator}$_executableName',
  ...
  test('stale preferred backend falls back safely', () async {
    ...
    final selection = File(
      '${temp.path}${Platform.pathSeparator}selection.json',
    );
